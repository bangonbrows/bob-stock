...
**Status: 🔍 SCOPE REVIEW R21 (R20 verdicts: BOTH BLOCK — AGY×2 + Codex×4 = 4 distinct REAL + note;
Q4v20 [§4/§8 collision parity] PASS BOTH (7 sub-questions now closed). The Live-presence DIRECTION is
CONFIRMED right by both ("the frozen workflow supports the premise; the problem is recovery after the
boundary") — the folds MAKE IT PRECISE: (1) the sweep keys on DURABLE PROVENANCE — the archive row's
`SourceId` (its exact original Live item id, immutable), NOT TransactionId (a late REPLAY re-inserts
the same TransactionId at a NEW item id ⇒ would falsely trip a TransactionId sweep; SourceId points
at the gone original ⇒ spared); (2) the reconcile now COMPLETES a published run's post-publish
Live-delete idempotently (delete Live ≤ the published snapshot's cutoff) BEFORE the sweep — so a
run that crashed MID-Live-delete no longer leaves published rows looking like residue; (3) N16's
stale "needs Published N17 or ctl-v1+head" text is removed (contradicted N10's not-in-Live retention);
(4) the cutover FENCES the push path during classification + completes published legacy runs'
Live-deletes; H14 stale marker-language cleaned. R19: 4 folded. R18-R1: 96 folded. Nothing built,
...
Live-deletes completed first, C2-R20-4).
...
- **H14 (Kunal-visible, C2-R16-1 revised by C2-R20):** the archive-cleanup job decides what is
  ...
  original is gone is kept as real history. So no hand-editable field can trick the job into
  deleting real records, and a genuinely-crashed run's duplicate is both harmless (the app dedups it
  automatically) and cleaned up. A person with direct database access could still forge live rows by
  hand (the same class of risk as editing the raw database) — covered by the planned server-side
  enforcement, acceptable for the trusted-staff alpha, and moot pre-launch since it's all trial
  data (D-C2-2).
...
## 14. Review questions (R21)

- **Q1v21 (SourceId sweep + reconcile completion):** C2-R20-1/-2 — with the sweep keyed on the
  archive row's immutable `SourceId` and the reconcile COMPLETING a published run's Live-delete
  first, is every case correct: crashed-prepublish (swept), published (spared), published-then-
  crashed-mid-Live-delete (reconcile completes ⇒ spared), late-replay (new item id ⇒ spared)? Is
  the reconcile-completion idempotent + guaranteed to run before the sweep every startup? Any
  remaining path where a published archive row is wrongly deleted or a residue row wrongly spared?
- **Q2v21 (one lifecycle):** C2-R20-3 — with N16's head-requirement removed, do N10, N16, §7b, and
  the cutover validator now state ONE consistent retention/selection lifecycle (retention by
  not-in-Live; export-active-selection by head)? Any surviving cross-section contradiction?
- **Q3v21 (cutover fence):** C2-R20-4 — does fencing the push path for the classification+seal
  interval (plus completing published legacy Live-deletes + SourceId keying) fully isolate the
  cutover? Any interleave or crash within the fenced interval that mis-seals or mis-classifies?
- **Q4v21 (whole-design closure — the convergence gate):** with all R1-R20 folds in place: any
