...
// W4.1-audit Codex-5: every bundle carries forId — the QUERY KEY the LA read it for (an empty read result
// otherwise proves nothing about the id the intent names).
const OFFICE_A = { forId: 'cockburn_office', store: { id: 'cockburn_office', isFranchise: true, isFranchiseOffice: true, active: true }, eras: [{ owner: 'fr_a', from: '2025-01-01T00:00:00Z', to: null }], pricing: { '*': [{ rate: 25, from: '2025-01-01T00:00:00Z', to: null }] } };
const OFFICE_B = { forId: 'b_office', store: { id: 'b_office', isFranchise: true, isFranchiseOffice: true, active: true }, eras: [{ owner: 'fr_b', from: '2025-01-01T00:00:00Z', to: null }], pricing: { '*': [{ rate: 30, from: '2025-01-01T00:00:00Z', to: null }] } };
const EMPTY_OFFICE = { forId: 'new_office', store: null, eras: [], pricing: {} };
const creds = [
...
ok('leg d/0: a CONSISTENT bundle for the WRONG office (forId+row = other_office) => OFFICE_STATE_MISMATCH', sixLeg({ ...OFFICE_A, forId: 'other_office', store: { ...OFFICE_A.store, id: 'other_office' } }) === 'OFFICE_STATE_MISMATCH');
ok('envelope: bundle row id != its own forId => OFFICE_ID_MISMATCH (Codex-5)', sixLeg({ ...OFFICE_A, store: { ...OFFICE_A.store, id: 'other_office' } }) === 'OFFICE_ID_MISMATCH');
ok('leg e: office open era owned by a DIFFERENT (existing) franchisee => OFFICE_STATE_MISMATCH', sixLeg({ ...OFFICE_A, eras: [{ owner: 'fr_x', from: '2025-01-01T00:00:00Z', to: null }] }, [{ franchiseeId: 'fr_a', officeStoreId: 'cockburn_office', officeUsername: 'fran_a' }, { franchiseeId: 'fr_x' }]) === 'OFFICE_STATE_MISMATCH');
...
ok('office store WITHOUT any era => STORE_ERA_MISMATCH (existence agreement mirrored)', T.planTopologyChange({ op: 'convert', storeId: 'boor', toFranchiseeId: 'fr_a' }, { ...stateHO, office: { forId: 'cockburn_office', store: OFFICE_A.store, eras: [], pricing: {} } }, NOW).reason === 'STORE_ERA_MISMATCH');
ok('orphan office pricing with NO office store => STORE_ERA_MISMATCH', T.planTopologyChange({ op: 'convert', storeId: 'boor', toFranchiseeId: 'fr_a' }, { ...stateHO, office: { forId: 'cockburn_office', store: null, eras: [], pricing: { '*': [{ rate: 25, from: '2025-01-01T00:00:00Z', to: null }] } } }, NOW).reason === 'STORE_ERA_MISMATCH');
...
ok('SR-77: an EXISTING store row at officeStoreId => OFFICE_STORE_ID_TAKEN', T.planTopologyChange({ op: 'onboard', storeId: 'newst', officeStoreId: 'cockburn_office', newFranchisee: { franchiseeId: 'fr_new', officeUsername: 'fran_new' }, rate: 20 }, ST({ store: null, franchisees: [{ franchiseeId: 'fr_a' }], office: OFFICE_A }), NOW).reason === 'OFFICE_STORE_ID_TAKEN');
ok('SR-77: orphan history at officeStoreId => STORE_ERA_MISMATCH', T.planTopologyChange({ op: 'onboard', storeId: 'newst', officeStoreId: 'new_office', newFranchisee: { franchiseeId: 'fr_new', officeUsername: 'fran_new' }, rate: 20 }, ST({ store: null, franchisees: [{ franchiseeId: 'fr_old' }], office: { forId: 'new_office', store: null, eras: [{ owner: 'fr_old', from: '2025-01-01T00:00:00Z', to: null }], pricing: {} } }), NOW).reason === 'STORE_ERA_MISMATCH');
...
// ── OS-W4.1 BUILD AUDIT round 1 (Codex ×6 + AGY ×1) — one probe per finding, proving each fix ───────────
console.log('== OS-W4.1 build-audit fixes (W41-A-C1..C5, W41-A-A1) ==');
// C1a: the fanout precondition captures every DECISION field, not just the written fields.
ok('C1a: fanout expect carries Role/franchiseeId/isStorePOS/isFranchiseOffice (the decision inputs)', (() => { const f = conv.plan.fanout.find(x => x.id === 'off_a'); return f.expect.Role === 'franchisee' && f.expect.franchiseeId === 'fr_a' && f.expect.isFranchiseOffice === true && f.expect.isStorePOS === null; })());
// C1b: createAccounts entries carry the ABSENCE precondition.
ok('C1b: onboard createAccounts entries carry expect.absent (franchisee + POS)', ob.plan.createAccounts.every(a => a.expect && a.expect.absent === true));
ok('C1b: create-HO POS account carries expect.absent', (() => { const p = T.planTopologyChange({ op: 'create', type: 'HO', storeId: 'brandnew2' }, ST({ store: null }), NOW); return p.ok && p.plan.createAccounts[0].expect.absent === true; })());
// C2 (Codex's exact repro): officeStoreId colliding with an EXISTING franchiseeId => USERNAME_TAKEN.
ok('C2: officeStoreId == an existing franchiseeId => USERNAME_TAKEN (entity ids join the namespace)', T.planTopologyChange({ op: 'onboard', storeId: 'new_store', officeStoreId: 'fr_existing', newFranchisee: { franchiseeId: 'fr_new', officeUsername: 'new_login' }, rate: 20 }, ST({ store: null, franchisees: [{ franchiseeId: 'fr_existing' }], office: { forId: 'fr_existing', store: null, eras: [], pricing: {} } }), NOW).reason === 'USERNAME_TAKEN');
// C3 (Codex's exact repro): the entity naming the TARGET as its own office => OFFICE_STATE_MISMATCH.
ok('C3: franchisee.officeStoreId == the target storeId => OFFICE_STATE_MISMATCH (no self-alias)', T.planTopologyChange({ op: 'convert', storeId: 'boor', toFranchiseeId: 'fr_a' }, ST({ store: { id: 'boor' }, eras: goodEras, creds: [posB, { id: 'off_a', Role: 'franchisee', StoreIds: ['boor'], franchiseeId: 'fr_a', isFranchiseOffice: true }], franchisees: [{ franchiseeId: 'fr_a', officeStoreId: 'boor' }], office: { forId: 'boor', store: { id: 'boor', isFranchise: true, isFranchiseOffice: true, active: true }, eras: [{ owner: 'fr_a', from: '2025-01-01T00:00:00Z', to: null }], pricing: { '*': [{ rate: 37, from: '2025-01-01T00:00:00Z', to: null }] } } }), NOW).reason === 'OFFICE_STATE_MISMATCH');
// C4a (Codex's exact repro): a FUTURE-dated open office era/rate is not "presently effective" — no clone.
ok('C4a: office era + rate opening in 2030 => OFFICE_STATE_MISMATCH (not effective at plan time)', T.planTopologyChange({ op: 'convert', storeId: 'boor', toFranchiseeId: 'fr_a' }, { ...stateHO, office: { ...OFFICE_A, eras: [{ owner: 'fr_a', from: '2030-01-01T00:00:00Z', to: null }], pricing: { '*': [{ rate: 37, from: '2030-01-01T00:00:00Z', to: null }] } } }, NOW).reason === 'OFFICE_STATE_MISMATCH');
// C4b (Codex's exact repro): the ORPHAN_ERA_OWNER rule now applies to the office surface.
ok('C4b: a supplied office bundle whose open era belongs to a GHOST franchisee => ORPHAN_ERA_OWNER', T.planTopologyChange({ op: 'buyback', storeId: 'boor' }, { ...stateFr, office: { ...OFFICE_A, eras: [{ owner: 'fr_ghost', from: '2025-01-01T00:00:00Z', to: null }] } }, NOW).reason === 'ORPHAN_ERA_OWNER');
// C5 (Codex's exact repro): an EMPTY absence-read must be FOR the requested id.
ok('C5: onboard office bundle read for a DIFFERENT key (forId x, intent claimed_y) => OFFICE_ID_MISMATCH', T.planTopologyChange({ op: 'onboard', storeId: 'newst', officeStoreId: 'claimed_y', newFranchisee: { franchiseeId: 'fr_new', officeUsername: 'fran_new' }, rate: 20 }, ST({ store: null, office: { forId: 'x', store: null, eras: [], pricing: {} } }), NOW).reason === 'OFFICE_ID_MISMATCH');
ok('C5: a bundle missing forId entirely => BAD_STATE', T.planTopologyChange({ op: 'convert', storeId: 'boor', toFranchiseeId: 'fr_a' }, { ...stateHO, office: { store: OFFICE_A.store, eras: OFFICE_A.eras, pricing: OFFICE_A.pricing } }, NOW).reason === 'BAD_STATE');
// A1 (AGY's exact repro): the four minted onboard identifiers are PAIRWISE distinct.
ok('A1: onboard with newFranchisee.franchiseeId == storeId => BAD_NEW_FRANCHISEE', T.planTopologyChange({ op: 'onboard', storeId: 'STORE_X', officeStoreId: 'OFFICE_X', rate: 15, newFranchisee: { franchiseeId: 'STORE_X', officeUsername: 'USER_X', displayName: 'Store X Franchisee' } }, ST({ store: null, office: { forId: 'OFFICE_X', store: null, eras: [], pricing: {} } }), NOW).reason === 'BAD_NEW_FRANCHISEE');
ok('A1: franchiseeId == officeUsername => BAD_NEW_FRANCHISEE', T.planTopologyChange({ op: 'onboard', storeId: 'newst', officeStoreId: 'new_office', newFranchisee: { franchiseeId: 'same_id', officeUsername: 'same_id' }, rate: 15 }, ST({ store: null, office: EMPTY_OFFICE }), NOW).reason === 'BAD_NEW_FRANCHISEE');
