...
async function runSmoke(repo) {
  ...
  try {
    ...
    { const { ctx, page } = await newPage(b); await page.route('**logic.azure.com**', r => r.fulfill({ status: 200, contentType: 'application/json', body: '{"ok":true,"items":[]}' })); await waitBoot(page, repo); await setup(page);
      const r = await page.evaluate(async () => { await Auth._cacheLocalVerifier('s214', 'secret-pw-8'); const good = await Auth._checkLocalVerifier('s214', 'secret-pw-8'); const bad = await Auth._checkLocalVerifier('s214', 'wrong'); const inLS = !!localStorage.getItem('bob_lverify_s214'); const raw = localStorage.getItem('bob_lverify_s214'); const isPbkdf2 = (()=>{try{const v=JSON.parse(raw);return v.iters>=100000 && /^[0-9a-f]{64}$/.test(v.hash) && !!v.salt;}catch(e){return false;}})(); return { good, bad, inLS, isPbkdf2 }; });
      rec('S-214', 'Chunk 9: offline PBKDF2 verifier round-trips (right pw ok, wrong rejected, >=100k iters, localStorage)', r.good === true && r.bad === false && r.inLS && r.isPbkdf2, `good=${r.good} bad=${r.bad} inLS=${r.inLS} pbkdf2=${r.isPbkdf2} (clean: true/false/true/true)`); await ctx.close(); }
