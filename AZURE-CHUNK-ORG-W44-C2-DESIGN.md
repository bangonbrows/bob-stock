...
**Status: 🔍 SCOPE REVIEW R22 (R21 verdicts: BOTH BLOCK — AGY×1 + Codex×4 = 4 distinct REAL; Q2v21
[one-lifecycle] + Q3v21 [cutover fence, AGY] confirmed. The Live-presence + SourceId direction holds;
the folds make the DELETION SET and its AUTHENTICATION precise: (1) the Live-delete/reconcile must
operate on the EXACT set of rows THIS run archived (query archive by `ArchiveRunId` → those exact
SourceIds), NOT scalar ≤cutoff — the unit-move archives high-id control/tombstone rows the scalar
delete would strand, then the sweep would destroy them (converged AGY-1≡Codex-2); (2) the per-run
archived MEMBERSHIP is AUTHENTICATED (fidelity hash extended to cover `SourceId`) so a tampered
SourceId column can't misdirect the sweep — mismatch ⇒ HALT (Codex-3); (3) push-v2 gains GLOBAL
idempotency (a TransactionId already in Live OR Archive ⇒ ack without a new Live row) — a late
REPLAY otherwise phantom-doubles on clients + later collides the archive (Codex-1); (4) §7b's
headless blocker only fires on an ABSENT target key — an explicit-null (withdrawn) head is
ADJUDICATED, never blocked (Codex-4). R20: 4 folded. R19-R1: 100 folded. Nothing built, nothing
deployed. D-C2-1..3 LOCKED — §12).**
...
NOT blocked by TOMBSTONE_PENDING_ADOPTION, C2-R21-4).
...
## 14. Review questions (R22)

- **Q1v22 (exact per-run deletion set):** C2-R21-1 — with the Live-delete/reconcile now deleting
  the EXACT SourceIds this run archived (by ArchiveRunId), not scalar ≤cutoff, is every case
  correct: a high-id unit-moved control (Live-deleted, so its archive copy is spared), a
  retain-window row (never archived ⇒ never Live-deleted), crashed-mid-delete (completed on
  restart)? Any run whose exact archived set is not reconstructible from its archive rows?
- **Q2v22 (authenticated SourceId):** C2-R21-2 — does covering `SourceId` in the run's fidelity
  hash make the sweep's provenance non-tamperable (a hand-edit ⇒ hash break ⇒ HALT)? Is the
  cross-check applied at BOTH the reconcile-delete and the sweep? Any window where an unauthenticated
  SourceId is trusted?
- **Q3v22 (global push idempotency):** C2-R21-3 — does checking Live AND Archive fully neutralize
  the replay (no phantom Live row ⇒ no client double-count ⇒ no later archive collision)? Is the
  exact-canonical-match ACK vs differing-match-reject correct, and does it interact safely with the
  §8 tombstone claim + the normal dedup?
- **Q4v22 (withdrawn-head blocker):** C2-R21-4 — does firing the headless blocker only on an ABSENT
  target key (explicit-null = adjudicated) correctly unblock every withdrawn/superseded target while
  still blocking a genuinely-unadjudicated headless row?
- **Q5v22 (whole-design closure — the convergence gate):** with all R1-R21 folds in place: any
