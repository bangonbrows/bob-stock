...
- **RUN it, don't just read it.** `azure-functions/src/functions/topology.js` is PURE — it takes the intent +
  ...
  decision engine directly (same as `accessPolicy.js`). Run `node test/topology-proof.js` (136 probes) AND
...
| `test/topology-proof.js` | OS-W2 logic proof — **136 probes, all pass on clean code** (43 original + 55 audit-fix regressions) |
| `AZURE-CHUNK-ORG-W1W2-AUDIT-RESPONSE.md` | The full round 1–11 finding→fix history — read before re-reporting |
