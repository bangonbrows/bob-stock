...
> **RE-CHECK CONTEXT (round 16, 2026-07-11):** this checkpoint has been through 13 fix rounds. AGY PASSed rounds
...
- **RUN it, don't just read it.** `azure-functions/src/functions/topology.js` is PURE — it takes the intent +
  ...
  decision engine directly (same as `accessPolicy.js`). Run `node test/topology-proof.js` (159 probes) AND
...
| `test/topology-proof.js` | OS-W2 logic proof — **159 probes, all pass on clean code** (43 original + 55 audit-fix regressions) |
| `AZURE-CHUNK-ORG-W1W2-AUDIT-RESPONSE.md` | The full round 1–13 finding + proactive sweep→fix history — read before re-reporting |
