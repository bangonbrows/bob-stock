...
const MUTATIONS = [
  ...
  { id: 'S-238', file: 'index.html',
    find: "return `<tr style=\"border-bottom:1px solid #f3f4f6\"><td style=\"padding:5px 6px\">${UI.esc(this._AP_SUDO_LABELS[k]||k)}</td><td style=\"text-align:center\">${floor?'🔒 Always':`<input type=\"checkbox\" ${on?'checked':''} onchange=\"Pages._apSetSudo('${k}',this.checked)\">`}</td></tr>`;",
    repl: "return `<tr style=\"border-bottom:1px solid #f3f4f6\"><td style=\"padding:5px 6px\">${UI.esc(this._AP_SUDO_LABELS[k]||k)}</td><td style=\"text-align:center\">${false?'🔒 Always':`<input type=\"checkbox\" ${on?'checked':''} onchange=\"Pages._apSetSudo('${k}',this.checked)\">`}</td></tr>`;",
    note: "AA (SR-1): the floor rows render as editable checkboxes -> the screen offers to relax always-password items (server still rejects, but the lock looks optional)" },
  ...
  { id: 'S-245', file: 'index.html',
    find: "if (Object.prototype.hasOwnProperty.call(this._RESERVED_CAP, cap)) return false;      // AA-11: reserved cap names denied, same as the server",
    repl: "if (Object.prototype.hasOwnProperty.call(this._RESERVED_CAP, cap)) return true;",
    note: "AA-11: the client RESERVED-cap guard fails OPEN -> a cap named __proto__/constructor is granted (also proves S-245 exercises the guard). NOTE: seeCharts-drop half of S-245 is structural (source), the RESERVED guard is the behavioural anchor." },
