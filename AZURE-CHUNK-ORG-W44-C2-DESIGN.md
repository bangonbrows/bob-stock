...
## R24 fold record (2026-07-25) — 🎉 AGY PASS (CONVERGED) · Codex BLOCK×1 final ("no other high-confidence finding")

| # | Finding (source) | Ground truth | Fold |
|---|------------------|--------------|------|
| C2-R24-1 | The mode-split's DISCRIMINATOR is downgradeable: "published = valid `PublishedSig`" — but PublishedSig is a separate DELETABLE field (and N10 elsewhere calls it advisory). Strip/corrupt an old published run's PublishedSig ⇒ it enters UNPUBLISHED SUBSET mode ⇒ per-member ∈-verification proves the OBSERVED rows were intended but CANNOT DETECT AN ABSENT one ⇒ a lost archived member (row 102) escapes the promised HALT and incomplete published history reaches settlement (Codex 1 — the FINAL finding; the marker-downgrade class of C2-R16-1 reapplied to the mode discriminator) | REAL — subset verification is one-directional; the mode gate must not hinge on a deletable marker | THE NON-DOWNGRADE DISCRIMINATOR (Codex's own correction): SUBSET MODE IS PERMITTED ONLY WHEN the PRE-PUBLICATION INVARIANT holds — EVERY intended SourceId in the signed `ArchiveMemberSourceIds` list STILL EXISTS IN LIVE (a genuinely unpublished/mid-copy run never deleted its Live originals, so ALL its intended members are still Live). Any intended member ABSENT from Live ⇒ this run cannot be unpublished ⇒ FULL-SET equality is REQUIRED (missing archived member ⇒ HALT + surface). A published run's originals are gone from Live ⇒ NO marker deletion can ever drop it into subset mode. The PublishedSig remains a fast-path hint only — the mode is decided by LEDGER STATE (the same non-forgeable principle as C2-R19-1) |
| C2-R24-N1 | H14 still says "integrity hash" where the mechanism is now the signed exact member list (Codex note) | Fair | H14: "sealed into each run's integrity hash" → "recorded in each run's signed member list" |

**Q1v24-Q2v24 outcomes:** Q1v24 — AGY: ALL five matrix cells verified PASS; Codex: four of five, the
published-with-stripped-marker downgrade was the escape (→ C2-R24-1); Q2v24 — **AGY: PASS, EXPLICIT
...
(non-empty production archive ⇒ HALT + surfaced decision, C2-R23-N1). R24 addition: the
NON-DOWNGRADE discriminator probe (a published run's `PublishedSig` stripped ⇒ its intended members
are GONE from Live ⇒ subset mode REFUSED ⇒ full-set equality runs ⇒ the missing archived member
HALTS — the Codex R24 repro verbatim; a genuine mid-copy crash [all intended members still Live] ⇒
subset mode permitted ⇒ cleaned without halt, C2-R24-1).
...
- **H14 (Kunal-visible, C2-R16-1 revised by C2-R20):** the archive-cleanup job decides what is
  ...
  original is gone is kept as real history. The identity it matches on is recorded in each run's
  signed member list, so a hand-edit of that field fails the list check and STOPS the
...
## 14. Review questions (R25)

- **Q1v25 (non-downgrade discriminator):** C2-R24-1 — with subset mode gated by the LEDGER-STATE
  invariant (every intended member still in Live) instead of the deletable PublishedSig, re-walk
  the matrix: stripped-marker published run (⇒ full-set ⇒ missing member HALTS), genuine mid-copy
  crash (⇒ subset ⇒ cleaned, no halt), tampered/planted residue (⇒ HALT), published deviated set
  (⇒ HALT). Any remaining downgrade or escape?
- **Q2v25 (whole-design closure — the convergence gate):** with all R1-R24 folds in place: any
