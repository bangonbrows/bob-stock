...
const MUTATIONS = [
  ...
  { id: 'S-272', file: 'sync.js',
    find: "      if (!Number.isFinite(pv) || pv < 0) return;\n      if (pv !== adopted) {",
    repl: "      if (Number.isFinite(pv) && pv > 0 && pv !== adopted) {",
    note: 'restores the old lax echo gate — missing/zero/empty/null pricingVersion clears stale + advances the horizon + opens the gate (Codex W42-A-C2)' },
