...
**Status: 🔍 SCOPE REVIEW R20 (R19 verdicts: BOTH BLOCK — AGY×2 + Codex×4 = 4 distinct REAL + proof
cleanup; Q4v19 [foreign-collision] PASS BOTH (6 sub-questions now closed). HEADLINE — both converged
on the SAME simplifying fix: the sweep's crash-proof must NOT rely on a DELETABLE signature (an
attacker deletes an old run's PublishedSig ⇒ published history looks crashed) — instead use ACTUAL
LEDGER STATE: an above-epoch archive row is RESIDUE iff its TransactionId is STILL PRESENT IN LIVE (a
crashed run never deleted its Live originals; a published run did). This DELETES the fragile
signature-absence sweep machinery: the sweep deletes iff in-Live (genuine duplicate), SPARES
everything not-in-Live (legit published history, direct-to-archive controls — all safe; the engine's
existing dual-list dedup makes any transient duplicate economically inert). Closes the reaper (F1)
AND the direct-control-brick (F2) at once. Also: the cutover clears in-Live legacy residue BEFORE
sealing (F3); §8's stale "collision stops the scrub" text is fixed (F4, §4 was already right). R18:
5 folded. R17-R1: 91 folded. Nothing built, nothing deployed. D-C2-1..3 LOCKED — §12).**
...
recovery text-parity probe (C2-R19-4).
...
## 14. Review questions (R20)

- **Q1v20 (live-presence sweep):** C2-R19-1 — is "delete an above-epoch archive row iff its
  TransactionId is still in Live; spare everything not-in-Live" fully non-forgeable and correct
  under every crash split? Verify the dependency: does the N10 reconcile ALWAYS complete a
  published run's post-publish Live-delete BEFORE the sweep runs, so a published row is reliably
  not-in-Live at sweep time? Any window where a legit published row is still in Live at sweep time
  (⇒ wrongly deleted) or a residue row is not-in-Live (⇒ wrongly spared, storage-collision)?
- **Q2v20 (direct-control retention):** C2-R19-2 — does not-in-Live-⇒-spared correctly retain
  EVERY legitimate direct-to-archive control (active, superseded, withdrawn, crashed-pre-publish)
  as append-only history, and does the export's head check still correctly select only the ACTIVE
  one? Can a forged control row (born in archive, valid-looking) be spared harmlessly (inert
  without a head)?
- **Q3v20 (cutover residue-clean):** C2-R19-3 — with the Live-presence residue cleanup run BEFORE
  sealing, is every crashed-legacy-run copy removed so none is sealed below the epoch? Any
  ordering hole between the cleanup, the seal, and re-enabling the C2 writer?
- **Q4v20 (§8 text parity):** C2-R19-4 — is the collision-recovery instruction now consistent
  across §4 and §8 (stop re-mint/retry only; scrub recovers)?
- **Q5v20 (whole-design closure — the convergence gate):** with all R1-R19 folds in place: any
