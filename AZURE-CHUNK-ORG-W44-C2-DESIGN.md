...
**Status: 🔍 SCOPE REVIEW R19 (R18 verdicts: BOTH BLOCK — AGY×2 + Codex×3 = 5 distinct REAL; Q2v18
[source-first pre-commit] + Q4v18 [collision recovery] PASS BOTH (2 more sub-questions closed);
Q3v18 SPLIT (Codex PASS / AGY found a real contradiction). The EPOCH MODEL is CONFIRMED sound by
both ("the item id is a suitable immutable classifier") — the folds COMPLETE it: the sweep now
deletes ONLY on POSITIVE crash proof (valid RecordSig + no valid PublishedSig + stale + non-current;
an absent/corrupt-RecordSig above-epoch row ⇒ HALT, not delete — closing the reinstated reaper); the
reader/sweep ACCEPT direct-to-archive SR-135 control rows (valid ctl-v1 seal + manifest head — they
have no ArchiveRunId); a QUIESCENT CUTOVER protocol captures+seals the epoch (disable old writer →
wait idle → seal → enable C2); the stale N17 "absent ⇒ legacy" sentence is DELETED; and the
foreign-main-row collision DELETES N18 + surfaces (the R17 "PRESERVE N18" was unimplementable vs the
collision-delete lifecycle). R17: 4 folded. R16-R1: 91 folded. Nothing built, nothing deployed.
D-C2-1..3 LOCKED — §12).**
...
durable registry `collision` state (C2-R15-3 + C2-R16-3, §4): STOP all re-mint/retry/scrub
re-attempts (no infinite 409), surface, and SELF-RESOLVE in a PINNED ORDER — **DELETE the N18 row
...
- **H14 (Kunal-visible, C2-R16-1):** the archive-cleanup job never auto-deletes archived history
  when the evidence is ambiguous (a missing or tampered "published" marker) — it STOPS and raises
  an alarm instead, and any stray un-cleaned data from a crashed run is simply ignored by every
  reader, so it can never corrupt a settlement. A person with direct list access could still
  delete or corrupt those markers by hand (the same class of risk as editing the raw database) —
  that's covered by the planned server-side enforcement and is acceptable for the trusted-staff
  alpha, especially since all pre-go-live data is trial data (D-C2-2).
...
## 14. Review questions (R19)

- **Q1v19 (positive-crash-proof sweep):** C2-R18-1 — with deletion now requiring a VALID RecordSig
  + no valid PublishedSig + stale + non-current (and absent/corrupt-RecordSig above-epoch ⇒ HALT),
  is the history-reaper fully closed (no list-access edit of an OLD published run's marker can
  delete its rows)? Is "natural residue always has a valid-RecordSig N17" actually guaranteed by
  the record-before-copy ordering under every crash split?
- **Q2v19 (direct-to-archive acceptance):** C2-R18-2 — does accepting a control row by its ctl-v1
  seal + live manifest head correctly admit EVERY legitimate direct-to-archive SR-135 correction
  (read + spared), and can a FORGED control row (no valid seal or no head) ever be admitted or
  spared? Is the head-liveness check race-safe against a concurrent withdraw?
- **Q3v19 (quiescent cutover):** C2-R18-3 — does disable→wait-idle→seal→enable prevent BOTH epoch
  races (in-flight old run's unpublished rows spared-as-legacy; post-capture old-run write above
  epoch)? Is driving a crashed in-flight run terminal before sealing well-defined?
- **Q4v19 (foreign-collision resolution):** C2-R18-5 — with the foreign-id-squat now deleting N18
  + surfacing (target present, re-issuable), is the contradiction gone, and is losing the device's
  deletion until Director re-issue an acceptable fail-closed posture (surfaced, never silent)?
- **Q5v19 (whole-design closure — the convergence gate):** with all R1-R18 folds in place: any
