...
> **RE-CHECK CONTEXT (round 21, 2026-07-12):** this checkpoint has been through 18 fix rounds. AGY PASSed rounds
...
> (map keys + requested key). **Round 16 (just fixed)**: interval endpoints (`from`/`to` on eras AND pricing) must
> now be ISO STRINGS — the shared `validIntervals` primitive type-checks before parsing, so a numeric `from:0` or
> a Date object no longer coerces into authoritative history. **Round 17 (just fixed)**: the string check is now
> STRICT ISO-UTC (`isIsoUtc`) — Date.parse-tolerated strings (`'0'`, `'June 1, 2025'`, date-only, non-UTC offsets)
> fail closed, and a ROUND-TRIP comparison rejects rolled-over impossible components (`2025-02-30` → Mar 2).
> **Round 18 (just fixed)**: `appendPricingForKey` now sweeps every series' VALUES (an untouched series with a
> dirty endpoint previously rode through the spread), and the READ path pre-emptively got the same whole-map
> value sweep (no resolving around a corrupt unrelated series). Every pricing surface now validates the whole
> map — keys AND values. The suite is now **191 probes** (`node test/topology-proof.js`). Do not re-report
...
- **RUN it, don't just read it.** `azure-functions/src/functions/topology.js` is PURE — it takes the intent +
  ...
  decision engine directly (same as `accessPolicy.js`). Run `node test/topology-proof.js` (191 probes) AND
...
| `test/topology-proof.js` | OS-W2 logic proof — **191 probes, all pass on clean code** (43 original + 55 audit-fix regressions) |
| `AZURE-CHUNK-ORG-W1W2-AUDIT-RESPONSE.md` | The full round 1–18 finding + proactive sweep→fix history — read before re-reporting |
