...
> **RE-CHECK CONTEXT (round 20, 2026-07-12):** this checkpoint has been through 17 fix rounds. AGY PASSed rounds
...
> fail closed, and a ROUND-TRIP comparison rejects rolled-over impossible components (`2025-02-30` → Mar 2). The
> suite is now **186 probes** (`node test/topology-proof.js`). Do not re-report anything already listed as fixed
> in the response doc unless you can show it still repros on the current tree (commit `f0ffe9b`).
...
- **RUN it, don't just read it.** `azure-functions/src/functions/topology.js` is PURE — it takes the intent +
  ...
  decision engine directly (same as `accessPolicy.js`). Run `node test/topology-proof.js` (186 probes) AND
...
| `test/topology-proof.js` | OS-W2 logic proof — **186 probes, all pass on clean code** (43 original + 55 audit-fix regressions) |
| `AZURE-CHUNK-ORG-W1W2-AUDIT-RESPONSE.md` | The full round 1–17 finding + proactive sweep→fix history — read before re-reporting |
