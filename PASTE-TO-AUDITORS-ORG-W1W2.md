...
> **RE-CHECK CONTEXT (round 12, 2026-07-11):** this checkpoint has been through 10 fix rounds. AGY PASSed rounds
...
> The suite is now **129 probes** (`node test/topology-proof.js`). Do not re-report anything already listed as
> fixed in the response doc unless you can show it still repros on the current tree (commit HEAD).
...
- **RUN it, don't just read it.** `azure-functions/src/functions/topology.js` is PURE — it takes the intent +
  ...
  decision engine directly (same as `accessPolicy.js`). Run `node test/topology-proof.js` (129 probes) AND
...
| `test/topology-proof.js` | OS-W2 logic proof — **129 probes, all pass on clean code** (43 original + 55 audit-fix regressions) |
| `AZURE-CHUNK-ORG-W1W2-AUDIT-RESPONSE.md` | The full round 1–10 finding→fix history — read before re-reporting |
