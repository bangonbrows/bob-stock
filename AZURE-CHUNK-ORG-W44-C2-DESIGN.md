...
**Status: 🔍 SCOPE REVIEW R23 (R22 verdicts: BOTH BLOCK — AGY×1 + Codex×2 = 2 distinct REAL (both
CONVERGED); Q1v22 [exact deletion set], Q3v22 [global idempotency concept], Q4v22 [withdrawn-head]
CONFIRMED. Both findings = the R21 folds under-specified in the NORMATIVE sections: (1) the "run's
fidelity hash authenticates SourceId" had NO DURABLE ARTIFACT (the fidelity hash is transient;
RecordSig didn't cover it) → N17 gains a signed `ArchiveMembersHash` (durable, RecordSig-covered,
written before copy) that the reconcile+sweep verify against; the LEGACY cutover explicitly BYPASSES
it (legacy runs have no such artifact; D-C2-2 trial data, production fresh) — converged AGY-1≡Codex-1;
(2) the global push idempotency lived only in the N9 table — the normative push §8 still said
"non-tombstone rows: pipeline untouched" and pinned no read order → the global Live→Archive
source-first check now applies to ALL rows in §8 (Codex-2). R21: 4 folded. R20-R1: 104 folded.
...
Live row, source-first Live→Archive, C2-R22-2).
...
## 14. Review questions (R23)

- **Q1v23 (durable membership authority):** C2-R22-1 — does the signed `ArchiveMembersHash` in N17
  give the reconcile-delete AND the sweep a durable, non-tamperable authority to verify the
  `ArchiveRunId==R` queried set (tamper ⇒ HALT; mid-delete crash recovers)? Is it written before
  copying, covered by RecordSig, and verified at BOTH sites? Any run whose membership can't be
  built/verified?
- **Q2v23 (legacy bypass):** C2-R22-1 — is bypassing the membership-hash authentication for pre-C2
  legacy rows (≤ epoch, no artifact) the right call — never HALT-bricking staging, never exercised
  in a fresh production cutover — and is the residual (unauthenticated legacy SourceIds) correctly
  bounded to trial data (D-C2-2) and documented (H14)?
- **Q3v23 (normative global idempotency):** C2-R22-2 — with the global Live→Archive source-first
  check now in the normative push §8 for ALL row types (tombstones delegating to the registry
  lifecycle after), is the replay fully closed for ordinary rows too, and is the read order/stability
  sufficient against a concurrent unit-move?
- **Q4v23 (whole-design closure — the convergence gate):** with all R1-R22 folds in place: any
