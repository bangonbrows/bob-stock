...
const MUTATIONS = [
  ...
  { id: 'S-150b', sentinel: 'S-150', file: 'index.html',
    find: "            prodDisc = (p && p.franchiseDiscount) ? p.franchiseDiscount : baseDisc;  // Wave M2 / GPTa-41: a stored 0 (no UI 0% option) = inherit the office default, not full price",
    repl: "            prodDisc = (p && p.franchiseDiscount != null) ? p.franchiseDiscount : baseDisc;",
    note: "franchise billing treats a legacy stored 0 as a real 0% discount -> office charged full price instead of inheriting its default (Wave M2 / GPTa-41). RE-ANCHORED 2026-07-16: OS-W4.2 moved this line into the invoice's legacy-fallback branch (assignment form, deeper indent) — the guarded behaviour is unchanged there" },
  ...
  { id: 'S-174', file: 'records.js',
    find: 'const disagree = Object.keys(lineMap).some(pid => lineMap[pid].qtys.size > 1 || lineMap[pid].stamps.size > 1);',
  ...
  { id: 'S-222', file: 'db.js',
    find: "          const keepTxn  = (t) => !!t && keep(t.storeId);",
    repl: "          const keepTxn  = (t) => !!t;",
    note: "Chunk 10 (anchor moved to purgeToScopeAtomic, OS-W3): the purge stops filtering the ledger -> out-of-scope store rows the device synced under a wider scope linger locally (P-13 residual leak)" },
  { id: 'S-223', file: 'sync.js',
    find: "    if (!sigChanged && topoWipe.length === 0 && !lateRows) return false;   // steady state — normal pull continues",
    repl: "    if (true) return false;",
    note: "Chunk 10 (anchor moved to the OS-W3 reconcile): _reconcileScope never detects a scope change -> a narrowed scope never purges/re-bootstraps; the device keeps serving out-of-scope local data" },
  ...
  // ─── OS-W3 (offline flush-before-purge + era re-bootstrap + topology hold) ──────────────────────────
  { id: 'S-250', file: 'sync.js',
    find: "    await this._drainPendingLocked();",
    repl: "    /* sabotaged: no flush */",
    note: "OS-W3-1: the flush-before-purge drain is skipped -> a pending offline row for a departing store is never pushed before reconcile (the purge then refuses, so the sentinel sees pushed=false; the ORIGINAL data-loss ordering, OS-SR-5)" },
  { id: 'S-251', file: 'db.js',
    find: "    if (!this.hasOutOfScopeRows([...rec])) return false;",
    repl: "    return false;   // sabotaged: detector blind",
    note: "OS-W3-2/11: the re-arm detector goes blind -> a follower write that lands AFTER the atomic purge commit is never detected; the device stays 'reconciled by signature' holding an out-of-scope row forever" },
  { id: 'S-252', file: 'sync.js',
    find: "        try { await DB.markTransactionsRejected(new Map(_rejected.map(t => [t.id, { code: 'EGRESS_INVALID', reason: 'excluded by client egress validation' }]))); } catch (e) { console.warn('[Sync] egress-reject flag failed (will retry next cycle):', e); }",
    repl: "        /* sabotaged: no durable egress rejection */",
    note: "OS-W3-6: egress-excluded rows are no longer durably _rejected -> a malformed local row the server will never see blocks the scope-purge pending guard forever (livelock; the sentinel's phase-2 purge never completes)" },
  { id: 'S-253', file: 'sync.js',
    find: "      const ok = await DB.purgeToScopeAtomic(scopeArr, { wipeStores: topoWipe });",
    repl: "      const ok = await DB.purgeToScopeAtomic(scopeArr, { wipeStores: [] });",
    note: "OS-W3-3/7: the era-bump wipe list is dropped -> an ownership change with unchanged StoreIds purges NOTHING and the new owner's device keeps the ex-owner's full local history (the POS old-era leak, OS-SR-6)" },
  { id: 'S-254', file: 'sync.js',
    find: "          if (remote.topologyPending === true) {",
    repl: "          if (false && remote.topologyPending === true) {",
    note: "OS-W3-8/12/13: the topology-hold branch is dead -> a quiesced store's hold envelope is treated as a normal empty pull; a maxId in the body advances the ledger cursor over withheld rows (skipped forever when the hold lifts)" },
  { id: 'S-255', file: 'sync.js',
    find: "    if (this._scopePurgePending || (() => { try { return localStorage.getItem('bob_scope_purge_pending') === '1'; } catch (e) { return false; } })()) {",
    repl: "    if (false) {",
    note: "OS-W3-10/15: pullSteps loses its purge-pending arm -> the step cursor advances while the ledger is unreconciled (a non-hold pull that aborts on purge failure no longer suppresses steps)" },
  { id: 'S-256', file: 'sync.js',
    find: "          this._reconcilePolicyPurge().catch(() => {});   // AA-05: retry a stuck cost purge every pull, version-independent",
    repl: "          /* sabotaged: policy purge starved */",
    note: "OS-W3-4/9: the page-1 policy purge stops running -> a stuck scope purge / topology hold starves the AA cost-visibility downgrade (a demoted user keeps elevated cost data; the starvation Codex R1 flagged)" },
  { id: 'S-257', file: 'db.js',
    find: "      if (ids.some(v => rec.has(v))) return;",
    repl: "      return; // sabotaged: guard never arms",
    note: "OS-W3-16: the DB-layer write guard goes inert -> a stale draft-transfer modal writes an out-of-scope row with no lock and no re-arm (the scheduleSync-bypass side door Codex R5 found)" },
  // ─── OS-W4.2 (the era-aware pricing lens) — one mutation per sentinel, S-261..S-270 ─────────────────
  { id: 'S-261', file: 'index.html',
    find: "      if(!this._validRate(q.rate))return false;",
    repl: "      if(false&&!this._validRate(q.rate))return false;",
    note: "W4-1: the client series validator stops checking rates -> a stored 999% resolves client-side while the REAL server validator rejects it; the lens/server parity sentinel flips on the one-sided edit" },
  { id: 'S-262', file: 'index.html',
    find: "            const r = Pricing.rateAsOf(office.id, t.productId, Date.parse(t.date + 'T00:00:00.000Z'), d.stores);",
    repl: "            const r = Pricing.rateAsOf(office.id, t.productId, Date.now(), d.stores);",
    note: "W4-2: the invoice resolves every line AS-OF TODAY instead of the line's own date -> a rate change today rewrites every PAST invoice again (the exact GAP-1 bug the lens exists to kill)" },
  { id: 'S-263', file: 'index.html',
    find: "  lensActive(){ const f=this._flags(); return f.activated||f.unresolved; },",
    repl: "  lensActive(){ return true; },",
    note: "W4-3: the lens claims to govern PRE-activation -> with no config the invoice fails closed instead of running today's computation (the byte-identical fallback regression breaks)" },
  { id: 'S-264', file: 'index.html',
    find: "    for(const q of series){ const f=Date.parse(q.from); if(dateMs<f)continue; const t=q.to==null?Infinity:Date.parse(q.to); if(dateMs<t)return q.rate; }",
    repl: "    for(const q of series){ const f=Date.parse(q.from); if(dateMs<f)continue; const t=q.to==null?Infinity:Date.parse(q.to); if(dateMs<=t)return q.rate; }",
    note: "W4-4: [from,to) becomes [from,to] -> a line dated exactly on an interval boundary resolves the OLD rate instead of the new one (boundary corruption on every rate change date)" },
  { id: 'S-265', file: 'index.html',
    find: "    if(!this.validConfig(cfg))return {error:'PRICING_DATA_ERROR'};   // P2: whole-config validation FIRST",
    repl: "    /* sabotaged: no whole-config validation */",
    note: "W4-6: whole-config validation is skipped -> a malformed sibling series no longer fails the store closed; resolution proceeds over a corrupt config (the exact fail-open the spec forbids)" },
  { id: 'S-266', file: 'sync.js',
    find: "        if (k === 'franchiseDiscount' && _holdPricingScalars) return;  // OS-W4.2 (SR-82): a LEADING publication's pricing scalars are held until the config catches up",
    repl: "        /* sabotaged: leading scalars apply */",
    note: "W4-9 (SR-82): a LEADING master_data's franchise-discount scalars apply immediately -> generic views show a publication the adopted history doesn't have yet (the exact-equality coherence breaks)" },
  { id: 'S-267', file: 'index.html',
    find: "      if(stRow&&stRow.isFranchise)return {error:'PRICING_DATA_ERROR'};",
    repl: "      /* sabotaged: franchise keys fall through */",
    note: "W4-11 (SR-10): an uncovered FRANCHISE billing key reads as honest NOT-SET instead of failing closed -> a config gap silently bills a franchise store 0% with no error surfaced as an error class" },
  { id: 'S-268', file: 'phase2.js',
    find: "    { const _pg = this._pricingSubmitGate(t.fromStoreId, t.toStoreId);   // OS-W4.2 P3/P5: submit = the pricing-commitment moment; gate BEFORE any mutation",
    repl: "    { const _pg = { ok: true };   // sabotaged: gate bypassed",
    note: "W4-16 (SR-49): the dormant-device gate is bypassed at submitDraft -> an offline/dormant HO device commits HO->franchise supply with no fresh pricing observation (the stale-scalar commit hole)" },
  { id: 'S-269', file: 'index.html',
    find: "    if(!f.activated&&!cfg)return {inactive:true};                    // P3: fallback ONLY when never served",
    repl: "    if(!cfg)return {inactive:true};                    // sabotaged: activation flag ignored",
    note: "W4-17 (SR-29): the durable activation flag is ignored -> an activated device that lost its config silently reverts to the scalar (the S-269 seed probe resolves the scalar 25 by luck but S-263's lens-off assertion and the activated+no-config fail-close both flip)" },
  { id: 'S-270', file: 'index.html',
    find: "      if(!Number.isFinite(h)||!(dateMs<h))return {error:'PRICING_STALE'};",
    repl: "      if(!Number.isFinite(h)||!(dateMs<=h))return {error:'PRICING_STALE'};",
    note: "W4-18 (SR-74): the stale horizon admits row instants AT the confirmation instant -> a boundary row a newer publication can reprice is admitted under stale (the strictly-before proof breaks)" },
  { id: 'S-271', file: 'index.html',
    find: "    if(typeof navigator!=='undefined'&&navigator.onLine===false)return {hold:'NO_FRESH_OBSERVATION'};   // OS-W42-AUDIT R1 (Codex C1): P5 pins a currently-HEALTHY connection at commit — a fresh flag from before the drop is not enough",
    repl: "    /* sabotaged: offline commit allowed */",
    note: 'commitGate loses the healthy-connection requirement — an offline device with a pre-drop fresh flag commits (Codex W42-A-C1)' },
  { id: 'S-272', file: 'sync.js',
    find: "      if (typeof rawPv !== 'number') return;\n      const pv = rawPv;",
    repl: "      const pv = Number(rawPv);   /* sabotaged: coercive versions accepted */",
    note: "type coercion returns — ''/null collapse to 0 and pass as the pre-activation statement (Codex W42-R2-C2)" },
  { id: 'S-276b', sentinel: 'S-276', file: 'phase2.js',
    find: "    item.sellAtSupply = sell; item.discAtSupply = disc; item.pricingVersion = av.pv; item.catalogueVersion = av.cv; item.basis = basis;",
    repl: "    item.sellAtSupply = sell; item.discAtSupply = disc; item.basis = basis;   /* sabotaged: authority versions dropped */",
    note: 'stamps mint WITHOUT the authority versions — the server attestation inputs are missing (W4.3 R1 Codex-1)' },
  { id: 'S-278c', sentinel: 'S-278', file: 'phase2.js',
    find: "        if (!_priceOk && !_isConsumable) return { ok: false, error: UI.productName(item.productId) + ' has no sell price in the catalogue — a Director must set it before franchise supply can be sent.' };",
    repl: "        /* sabotaged: priceless retail stamps $0 */",
    note: 'a retail product with no catalogue price commits a genuine $0 stamp instead of failing closed (W4.3 R1 Codex-3)' },
  { id: 'S-280b', sentinel: 'S-280', file: 'records.js',
    find: "      if (l.sellAtSupply != null && l.discAtSupply != null) e.stamps.add(l.sellAtSupply + '|' + l.discAtSupply + '|' + (l.pricingVersion != null ? l.pricingVersion : '') + '|' + (l.catalogueVersion != null ? l.catalogueVersion : '') + '|' + (l.basis || ''));",
    repl: "      if (l.sellAtSupply != null && l.discAtSupply != null) e.stamps.add(l.sellAtSupply + '|' + l.discAtSupply + '|' + (l.basis || ''));   /* sabotaged: versions out of the identity */",
    note: 'a version-only stamp difference folds as a benign duplicate (W4.3 R1 Codex-1 identity half)' },
  { id: 'S-281c', sentinel: 'S-281', file: 'index.html',
    find: "            if (_it._stampsUntrusted) {",
    repl: "            if (false) {",
    note: 'backfill-only stamps are TRUSTED as valuation evidence — restored-transfer invoices diverge from settlement (W4.3 R1 Codex-2)' },
  { id: 'S-281d', sentinel: 'S-281', file: 'index.html',
    find: "      return _tr ? _tr.fromStoreId === 'head_office' : false;",
    repl: "      if (_tr) return _tr.fromStoreId === 'head_office';   /* sabotaged: absent record falls through to the reason text */",
    note: 'a franchise store NAMED like head office regex-matches store-to-store transfers into the invoice (W4.3 R1 AGY-1)' },
  { id: 'S-281g', sentinel: 'S-281', file: 'index.html',
    find: "          if (_it && _itN > 0 && _itN < 4) { prodDisc = 0; lineErr = 'STAMP_ERROR'; _stamped = true; }   // OS-W43-R3 (Codex): a PARTIAL item tuple is malformed — fail closed, never \"transfer-stamped\", never the lens",
    repl: "          if (false) { }   /* sabotaged: partial item tuples bill as stamped */",
    note: 'a money-only transfer item bills as valid transfer-stamped data (W4.3 R3 Codex)' },
  { id: 'S-281e', sentinel: 'S-281', file: 'index.html',
    find: "        if (_stamped && !lineErr && prodDisc === 0) lineErr = 'NOT_SET';",
    repl: "        /* sabotaged: silent stamped 0% */",
    note: 'a stamped 0% renders as a silent full-price line instead of the loud NOT-SET surfacing (W4.3 R1 AGY-2)' },
  { id: 'S-282d', sentinel: 'S-282', file: 'records.js',
    find: "    if (recordType === 'delivery') return this._canonicalDeliveryV4(snap);",
    repl: "    if (recordType === 'delivery') return Object.assign({}, snap, this._canonicalDeliveryV4(snap));   /* sabotaged: unknown delivery keys leak */",
    note: 'delivery canonicalization preserves unknown future fields — schema evolution falsely diverges backfills (W4.3 R1 Codex-4/AGY-3)' },
  { id: 'S-272b', sentinel: 'S-272', file: 'sync.js',
    find: "      if (pv === 0 && (_act || adopted > 0)) return;   // a \"no pricing\" claim against an activated/adopted device confirms nothing (R2-C2)",
    repl: "      /* sabotaged: version-0 claims trusted */",
    note: 'a version-0 echo freshens an ACTIVATED device that lost its config (Codex W42-R2-C2 second half)' },
  { id: 'S-273b', sentinel: 'S-273', file: 'sync.js',
    find: "    if (adoptedV !== echoedV || _durV !== echoedV || this._pricingFresh !== true) return { ok: false, reason: 'echo-not-adopted' };",
    repl: "    if (adoptedV < echoedV) return { ok: false, reason: 'echo-not-adopted' };   /* sabotaged: the weak R1 check */",
    note: 'writer success for a rollback echo or a non-durable adoption (Codex W42-R2-C3)' },
  { id: 'S-274b', sentinel: 'S-274', file: 'index.html',
    find: "    }catch(err){ try{ if(!_preUnres) localStorage.removeItem('bob_pricing_unresolved'); if(_preVer!=null) localStorage.setItem('bob_pricing_ver',_preVer); }catch(e){} UI.toast('Restore failed: '+err.message,'error'); return false; }",
    repl: "    }catch(err){ UI.toast('Restore failed: '+err.message,'error'); return false; }   /* sabotaged: no rollback */",
    note: 'a failed restore write strands the hold + loses the durable marker (Codex W42-R2-C4)' },
  { id: 'S-275', file: 'sync.js',
    find: "            this._invalidatePricingFresh();   // OS-W42-AUDIT R2 (Codex C1): the heartbeat DEMOTION is a leadership loss — the demoted tab stops pulling, so its freshness fact would otherwise grow stale forever",
    repl: "            /* sabotaged: demotion keeps freshness */",
    note: 'the heartbeat-demoted old leader keeps a freshness fact it can never refresh (Codex W42-R2-C1)' },
  { id: 'S-275b', sentinel: 'S-275', file: 'sync.js',
    find: "          this._invalidatePricingFresh();   // OS-W42-AUDIT R2 (Codex C1): standing down = a leadership loss — the pull loop is leader-only, a freshness fact must not outlive it",
    repl: "          /* sabotaged: stand-down keeps freshness */",
    note: 'the leader-exists stand-down keeps a freshness fact (Codex W42-R2-C1 second branch)' },
  { id: 'S-275c', sentinel: 'S-275', file: 'sync.js',
    find: "              this._invalidatePricingFresh();   // OS-W42-AUDIT R3 (Codex C1): LOSING the election tiebreak is a leadership loss too — a pre-election observation must not outlive it",
    repl: "              /* sabotaged: tiebreak loser keeps freshness */",
    note: 'the election-tiebreak loser keeps a pre-election freshness fact it can never refresh (Codex W42-R3-C1)' },
  { id: 'S-275d', sentinel: 'S-275', file: 'sync.js',
    find: "        if (!this._isLeader) this._invalidatePricingFresh();   // OS-W42-AUDIT R3 (sibling of Codex C1, found by inventory): a claim ABANDONED to another leader's mid-claim heartbeat is a leadership loss the heartbeat branch never invalidates (it only runs on leaders)",
    repl: "        /* sabotaged: abandoned claim keeps freshness */",
    note: 'a claim abandoned to a live leader keeps its freshness fact (R3 sibling path)' },
  { id: 'S-275e', sentinel: 'S-275', file: 'sync.js',
    find: "    this._invalidatePricingFresh();   // OS-W42-AUDIT R3 (sibling of Codex C1, found by inventory): a stopped sync can observe nothing — no freshness fact survives teardown",
    repl: "    /* sabotaged: stop keeps freshness */",
    note: 'sync teardown keeps a freshness fact (R3 sibling path)' },
  { id: 'S-275f', sentinel: 'S-275', file: 'sync.js',
    find: "    this._invalidatePricingFresh();   // OS-W42-AUDIT pre-R4 (the family's 401 sibling): an auth-paused device stops pulling — no freshness fact survives the pause",
    repl: "    /* sabotaged: 401 pause keeps freshness */",
    note: 'an auth-paused (401) device keeps its freshness fact while never pulling again (the pre-R4 sibling)' },
  { id: 'S-275k', sentinel: 'S-275', file: 'sync.js',
    find: "    this._invalidatePricingFresh();   // OS-W42-AUDIT R5 (AGY-1): logout structurally HALTS observation (pull() no-ops without URLs) — the freshness fact must not survive into a later re-login whose first fetch fails silently",
    repl: "    /* sabotaged: logout keeps freshness */",
    note: 'a logged-out tab keeps its freshness fact; a later re-login with a silently failed first fetch inherits it (AGY W42-R5-1)' },
  { id: 'S-276', file: 'phase2.js',
    find: "    // OS-W4.3 P1: the draft SUBMIT is the pricing-commitment moment — stamp now; restore the snapshot on reject\n    { const _st = this._stampItems(t);\n      if (!_st.ok) { Object.keys(t).forEach(k => delete t[k]); Object.assign(t, snapshot); return { ok:false, error:_st.error }; } }",
    repl: "    /* sabotaged: no stamps at draft submit */",
    note: 'the draft-submit path never stamps — items ship stampless and the invoice falls back to live pricing (W4.3 P1)' },
  { id: 'S-277', file: 'sync.js',
    find: "    if (present !== 4) return null;                                     // OS-W43-R1 (Codex-1): ALL FOUR authority fields or none — a partial tuple = malformed",
    repl: "    if (present !== 4) return { absent: true };   /* sabotaged: partial tuples pass as legacy */",
    note: 'a one-sided stamp ingests as a legacy row instead of rejecting — a malformed money claim becomes durable truth (W4.3 P6)' },
  { id: 'S-278', file: 'phase2.js',
    find: "    // OS-W4.3 P1: a non-draft create IS the submit — stamp the items now (a pricing failure rejects, nothing written)\n    { const _st = this._stampItems(transfer); if (!_st.ok) return { ok:false, error:_st.error }; }",
    repl: "    /* sabotaged: no stamps at create-submit */",
    note: 'the non-draft create path never stamps — billing rows carry no frozen money (W4.3 P1)' },
  { id: 'S-278b', sentinel: 'S-278', file: 'phase2.js',
    find: "        if (it && this._fullTuple(it)) { x.sellAtSupply = it.sellAtSupply; x.discAtSupply = it.discAtSupply; x.pricingVersion = it.pricingVersion; x.catalogueVersion = it.catalogueVersion; }   // OS-W43-R3: full tuple or nothing — never default a version",
    repl: "        /* sabotaged: rows never inherit item stamps */",
    note: 'top-up/return/receive rows stop inheriting the item stamps — row-level valuation loses the frozen money (W4.3 P2)' },
  { id: 'S-279', file: 'phase2.js',
    find: "      const submitMs = Date.parse(t.submittedAt || t.date || t.createdAt || '') || Date.now();   // OS-W43-R2 (Codex-3): the SUBMIT instant — submittedAt (W4 rows/steps), else date (set at submit in BOTH eras), createdAt last (draft creation, the bug Codex reproduced)",
    repl: "      const submitMs = Date.now();   /* sabotaged: receive-day rate */",
    note: 'stamp-at-receive resolves the discount at RECEIVE-day instead of the submit instant — in-transit transfers re-price in flight (W4.3 P4)' },
  { id: 'S-279b', sentinel: 'S-279', file: 'phase2.js',
    find: "      const submitMs = Date.parse(t.submittedAt || t.date || t.createdAt || '') || Date.now();   // OS-W43-R2 (Codex-3): the SUBMIT instant — submittedAt (W4 rows/steps), else date (set at submit in BOTH eras), createdAt last (draft creation, the bug Codex reproduced)",
    repl: "      const submitMs = Date.parse(t.createdAt || t.date || '') || Date.now();   /* sabotaged: draft-creation rate */",
    note: 'stamp-at-receive resolves at DRAFT-CREATION time — a stale draft submitted across a rate change stamps the wrong era (W4.3 R2 Codex-3)' },
  { id: 'S-280', file: 'records.js',
    find: "      if (l.sellAtSupply != null && l.discAtSupply != null) e.stamps.add(l.sellAtSupply + '|' + l.discAtSupply + '|' + (l.pricingVersion != null ? l.pricingVersion : '') + '|' + (l.catalogueVersion != null ? l.catalogueVersion : '') + '|' + (l.basis || ''));",
    repl: "      /* sabotaged: money out of the conflict identity */",
    note: 'equal-qty receives with DIFFERING stamps fold as a benign duplicate — billing becomes sync-order-dependent (W4.3 SR-66)' },
  { id: 'S-281', file: 'index.html',
    find: "            } else { sell = _it.sellAtSupply; prodDisc = _it.discAtSupply; rateSource = 'transfer-stamped'; _stamped = true; }",
    repl: "            } else { /* sabotaged: no item-stamp valuation tier */ }",
    note: 'an unstamped row of a stamped transfer values at the LENS instead of the submit stamps — the stale-receiver row bills differently per device (W4.3 SR-97/114)' },
  { id: 'S-281b', sentinel: 'S-281', file: 'records.js',
    find: "      else if (!_lnStamped && !_itemStamped && !item.basis) { item.basis = 'legacy-lens'; }",
    repl: "      else if (!_lnStamped) { item.basis = 'legacy-lens'; delete item.sellAtSupply; delete item.discAtSupply; }   /* sabotaged: the R7 downgrade bug restored */",
    note: 'a basis-less (pre-W4) receive silently DOWNGRADES a stamped submit to legacy-lens — the exact SR-97 regression (W4.3 P3)' },
  { id: 'S-282', file: 'records.js',
    find: "  _canonicalSerialize(v) {\n    if (v === null || typeof v !== 'object') return JSON.stringify(v === undefined ? null : v);\n    if (Array.isArray(v)) return '[' + v.map(x => this._canonicalSerialize(x)).join(',') + ']';\n    const keys = Object.keys(v).filter(k => v[k] !== undefined).sort();\n    return '{' + keys.map(k => JSON.stringify(k) + ':' + this._canonicalSerialize(v[k])).join(',') + '}';\n  },",
    repl: "  _canonicalSerialize(v) { try { return JSON.stringify(v, Object.keys(v || {}).sort()); } catch (e) { return String(v); } },   /* sabotaged: the old replacer trick */",
    note: 'the serializer reverts to the replacer trick — nested item stamps become invisible to the divergence hash ($100 vs $150 hash identically, the pre-existing D4-I blindness) (W4.3 SR-105)' },
  { id: 'S-282b', sentinel: 'S-282', file: 'records.js',
    find: "    if (recordType === 'transfer') return this._canonicalTransferV4(snap);",
    repl: "    if (recordType === 'transfer') return Object.assign({}, snap, this._canonicalTransferV4(snap));   /* sabotaged: unknown keys leak into the hash */",
    note: 'strict schema projection lost — a future-version field falsely diverges a W4 fold (W4.3 SR-128)' },
  { id: 'S-282c', sentinel: 'S-282', file: 'records.js',
    find: "      const settled = new Set((lastRes && lastRes.payload && lastRes.payload.resolvesBackfillHashes) || []);",
    repl: "      const settled = new Set();   /* sabotaged: resolves never settle divergence */",
    note: 'a Director-resolved backfill divergence re-flags forever — the record can never converge (W4.3 SR-106)' },
  { id: 'S-275i', sentinel: 'S-275', file: 'sync.js',
    find: "          this._invalidatePricingFresh();   // OS-W42-AUDIT R4 (AGY-1): the LEAVING notice is the stop-observing moment — the epoch advances NOW, not when the claim later resolves",
    repl: "          /* sabotaged: leaving notice keeps freshness */",
    note: 'the leader-leaving handoff window lets a late response resurrect freshness (AGY W42-R4-1)' },
  { id: 'S-275j', sentinel: 'S-275', file: 'sync.js',
    find: "        this._invalidatePricingFresh();   // OS-W42-AUDIT R4 (AGY-2): timeout DETECTION is the stop-observing moment — observation ended up to LEADER_TIMEOUT ago; the epoch advances before any claim",
    repl: "        /* sabotaged: timeout detection keeps freshness */",
    note: 'the heartbeat-timeout detection window lets a late response resurrect freshness (AGY W42-R4-2)' },
  { id: 'S-275g', sentinel: 'S-275', file: 'sync.js',
    find: "    if ((this._pricingEpoch || 0) !== _pep) this._pricingFresh = false;   // OS-W42-AUDIT pre-R4b (the late-response race): this response predates an invalidating event — it proves nothing about NOW",
    repl: "    /* sabotaged: late writer echo resurrects freshness */",
    note: 'a pricing-change response already in flight at an invalidating event resurrects freshness (the late-response race, writer path)' },
  { id: 'S-275h', sentinel: 'S-275', file: 'sync.js',
    find: "      if ((this._pricingEpoch || 0) !== _pep) this._pricingFresh = false;   // OS-W42-AUDIT pre-R4b (the late-response race): a config response from before an invalidating event must not resurrect freshness",
    repl: "      /* sabotaged: late config response resurrects freshness */",
    note: 'a config response already in flight at an invalidating event resurrects freshness (the late-response race, config path)' },
  { id: 'S-273', file: 'sync.js',
    find: "    if (j.config == null) return { ok: false, reason: 'no-echo' };",
    repl: "    if (j.config == null) return { ok: true };   /* sabotaged: success without the echoed publication */",
    note: 'the pricing writer reports success with no echoed config adopted — success toast while the lens and scalar sit unchanged (Codex W42-A-C4)' },
  { id: 'S-274', file: 'phase2.js',
    find: "      const fromWh = fromSt && !fromSt.isFranchise && fromSt.type === 'warehouse';\n      if (!((fromWh || fromStoreId === 'head_office') && toSt && toSt.isFranchise)) return { ok: true };",
    repl: "      if (!(fromSt && !fromSt.isFranchise && fromSt.type === 'warehouse' && toSt && toSt.isFranchise)) return { ok: true };   /* sabotaged: the old predicate */",
    note: 'the submit gate loses the billing-key leg — a billed head_office transfer with a missing store row skips the gate (AGY-3)' },

  { id: 'S-258', file: 'db.js',
    find: "            throw new Error('SCOPE_PURGE_PENDING_ROWS');",
    repl: "            console.warn('sabotaged: predicate disabled');",
    note: "OS-W3-2/5: the atomic purge's pending predicate is disabled -> the purge silently DROPS un-pushed rows (the core data-loss invariant; belt-and-braces gone)" },
  { id: 'S-259', file: 'sync.js',
